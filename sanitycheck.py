...
def run_sanity_check(test_dir):  # noqa: C901
    ...
    if not test_functions_for_post:
        ...
        if not TEST_FOR_POST_METHOD_RESPONSE_BODY:
            print(FAIL_COLOR + f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(FAIL_COLOR + "One or more of your test cases for POST() do not seem to be testing the \
            contents of the response.\n")

        if len(test_functions_for_post) >= 2 and COUNT_POST_METHOD_TEST_FOR_INFERENCE_RESULT < 2:
            print(FAIL_COLOR+f"[{WARNING_COUNT}]")
            WARNING_COUNT += 1
            print(FAIL_COLOR + "You do not seem to have TWO separate test cases, one for each possible \
